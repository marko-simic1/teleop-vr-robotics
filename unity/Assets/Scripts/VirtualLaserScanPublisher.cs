...
public class VirtualLaserScanPublisher : MonoBehaviour
{
    ...
    [Header("Raycast")]
    public Transform laserOrigin; // Ovdje prevuci samog Robota
    ...
    void PublishScan()
    {
        ...
        {
            float a = angleMin + i * angleInc; // radijani

            // Izračun smjera zrake
            ...
            if (Physics.Raycast(origin, dir, out RaycastHit hit, rangeMax, obstacleLayers))
            {
                ...
                // Debug linija (vidiš je u Scene view)
                Debug.DrawRay(origin, dir * d, Color.red, 0.11f);
            }
            else
            {
                ranges[i] = float.PositiveInfinity; // Nema prepreke
            }
        }
