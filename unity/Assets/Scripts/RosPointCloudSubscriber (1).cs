...
public class RtabmapCloudMapRenderer : MonoBehaviour
{
    ...
    bool printedFirstMessage = false;
    ...
    void Update()
    {
        ...
        Debug.Log($"[RTAB-Map] Mesh updated with {count} points");
    ...
    void CloudCallback(PointCloud2Msg msg)
    {
        ...
        if (!printedFirstMessage)
        {
            Debug.Log($"[RTAB-Map] First PointCloud2 received on {topicName}");
            printedFirstMessage = true;
        }
        ...
        Debug.Log(
            $"[RTAB-Map] Cloud msg #{messageCount} | " +
            $"Raw points: {total} | " +
            $"Used points: {vertices.Count}"
        );
